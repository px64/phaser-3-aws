...
import { characters } from './BaseScene.js';
    ...
    export function displayTutorial(scene) {
        ...
        if (scene.currentTutorialIndex < nextScreenTutorial.length) {
            ...
            if (!backdrop) {
                backdrop = scene.add.rectangle(scene.cameras.main.width/4, scene.cameras.main.height/2, scene.cameras.main.width/2, scene.cameras.main.height/2-100, 0x000000, 0).setOrigin(0, 0).setInteractive().setDepth(0);
            }
